...
set output '| epstopdf --filter --outfile=for_He_Zhang_new_LP_RNAfold_MEA_sens.pdf' 
...
set ylabel "Sensitivity" offset 1.5, 0
...
plot 'new_MEA_PPV_Sens_raw_data.csv' using 4:xticlabels(1) lc rgb 'orange' fillstyle pattern 4 title "Vienna RNAfold MFE", \
     '' using 7:xticlabels(1) lc rgb 'orange' fillstyle pattern 3 title "LinearFold-V MFE", \
     '' using 10:xticlabels(1) lc rgb 'blue' fillstyle pattern 4 title "Vienna RNAfold + MEA ({/:Italic {/Symbol g}=}1.5)", \
     '' using 13:xticlabels(1) lc rgb 'blue' fillstyle pattern 3 title "LinearPartition + MEA ({/:Italic {/Symbol g}=}1.5)", \
